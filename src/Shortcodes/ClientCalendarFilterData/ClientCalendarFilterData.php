...
namespace LWM\Disc\Shortcodes\ClientCalendarFilterData;

use LWM\Disc\Shortcodes\LWMDisclaimerShortcode;
use LWM\Disc\Utilities\Helpers;

if (!class_exists('LWM\Disc\Shortcodes\ClientCalendar\ClientCalendarFilterData')) {
    /**
     * Class ClientCalendarFilterData
     *
     * @package LWM\Disc\Shortcodes\ClientCalendarFilterData
     */
    class ClientCalendarFilterData extends LWMDisclaimerShortcode
    {
        ...
        {
            return 'getNotionClientCalendarFilterData';
        }
        ...
        public function init($atts, $shortcode_content = null)
        {
            ...
            if (!empty($this->json)) {
                ...
                $this->setWholeData();
                $removed_keys          = $this->getRemovedKeys($this->manageWholeDataFilter);
                $filtered_tokens       = $this->getFilteredTokens($this->manageWholeDataFilter, $removed_keys);
                $filtered_token_string = $this->getFilteredTokenString($filtered_tokens);
                $content               = sprintf(
        ...
        private function setWholeData()
        {
            ...
            if ($records) {
                foreach ($records as $index => $record) {
                    if (!$this->hasStatus($record->name) || 'Yes' === $record->payExpected) {
                        $end_key = Helpers::convertStringToDate($record->dateRange->endDate);
                        if (!empty($record->clientTickerDb)) {
                            if ($this->hasTickerValue($record)) {
                                // set ticker values in a single line
                                $key_tickers                                 = $this->getCombinedTickerValue($record);
                                $manage_whole_data[$key_tickers][$end_key][] = $record;
                            } elseif ('' !== $record->otherName) {
                                $manage_whole_data['otherkey'][$end_key][] = $record;
                            }
            ...
            $this->manageWholeDataFilter = $manage_whole_data;
        }
    }
}
